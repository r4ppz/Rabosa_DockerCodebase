...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $stmt = db()->prepare(
    'INSERT INTO reservations (guest_name, room_type, check_in, check_out, employee_id, status)
         VALUES (?, ?, ?, ?, ?, ?)'
  );
  $stmt->execute([$guestName, $roomType, $checkIn, $checkOut, $employeeId, $status]);

  $_SESSION['flash'] = ['type' => 'success', 'message' => 'Reservation #' . (int) db()->lastInsertId() . ' created successfully.'];
  ...
  <div class="col-lg-8">
    <div class="card shadow-sm">
      <div class="card-header bg-primary text-white">
        <h5 class="mb-0 fw-semibold">Create Reservation</h5>
      </div>
      <div class="card-body">
          ...
          <div class="row g-3">
            <div class="col-md-6">
              ...
              <input type="text" class="form-control" id="guest_name" name="guest_name"
                value="<?= htmlspecialchars($old['guest_name'] ?? '') ?>" required>
            </div>
            <div class="col-md-6">
            ...
            <div class="col-md-6">
              ...
              <input type="date" class="form-control" id="check_in" name="check_in"
                value="<?= htmlspecialchars($old['check_in'] ?? '') ?>" required>
            </div>
            <div class="col-md-6">
              <label for="check_out" class="form-label">Check-Out Date</label>
              <input type="date" class="form-control" id="check_out" name="check_out"
                value="<?= htmlspecialchars($old['check_out'] ?? '') ?>" required>
            </div>
            <div class="col-md-6">
